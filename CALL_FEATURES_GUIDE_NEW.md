...
3. Server `MS-consume` path now has H264 profile-level fallback for Web ↔ Flutter interop.
...
5. Keep codec negotiation flexible (do not force only one video codec if rooms include mixed clients).
...
- when `canConsume` fails for H264 profile mismatch, server retries with a relaxed H264 capability (drops `profile-level-id`) to improve Web ↔ Flutter compatibility
...
- Server consume path includes H264 profile-level fallback (if strict profile match fails once, backend retries with relaxed H264 params).
...
1. Verify both H264 and VP8 are present in router RTP capabilities
2. Compare client video codec parameters, especially H264 `profile-level-id`
3. Confirm backend includes H264 relaxed consume fallback (profile-level-id removed once on retry)
4. Re-check that client sends unmodified `device.rtpCapabilities` to `MS-consume`
